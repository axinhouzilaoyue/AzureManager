...
export async function initializeDatabase(db: SqlDatabase): Promise<void> {
  await db.batch([
    ...
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_accounts_subscription_id ON accounts(subscription_id)`),
    ...
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_tasks_account_id ON tasks(account_id)`),
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_tasks_status ON tasks(status)`),
    db.prepare(`CREATE TABLE IF NOT EXISTS task_logs (
    ...
    db.prepare(`CREATE INDEX IF NOT EXISTS idx_task_logs_task_id ON task_logs(task_id, id)`),
